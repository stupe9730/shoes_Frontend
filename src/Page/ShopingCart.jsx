...
const ShopingCart = () => {
  ...
  const [CartData, { data, isSuccess: datagetSuccess, isLoading: cardLoad }] =
    useLazyGetCartQuery();
  console.log(data);
  const [removecart, { isSuccess, isLoading }] = useLazyRemovecartQuery();
  const subtotal =
    data &&
  ...
  return (
    <>
      {cardLoad && isLoading && <Loader />}
      <div className="dark:bg-black  h-full md:h-full">
                      ...
                      {data &&
                        data.map((item) => (
                          ...
                          >
                            <th
                              scope="row"
                              className="sm:px-6 py-2 text-gray-900 font-normal whitespace-nowrap "
                            >
                              <div className="flex dark:text-white  gap-3">
                                <p className="my-1 sm:hidden bg-pink-400  font-mono left-28  absolute text-center font-bold pt-1 h-7 w-7 rounded-full">
                                  {item.qut}
                                </p>
                                <img
                                  src={`  ${item.img}`}
                                ...
                                />
                                <div>
                                  <p className="my-1  text-[14px]  font-bold sm:text-[15px]">
                                    {item.name}
                                  </p>
