...
const Admin = () => {
  ...
  const [addProduct, { isLoading: addLoading, isSuccess: addSuccess }] =
    useAddAdminProductMutation();
  const [getProducts, { data, getSuccess, isError, error, isLoading }] =
    useLazyGetAdminProductQuery();
  const [
    deleteProduct,
  ...
  const [
    updateProduct,
    { isLoading: updatedLoading, isSuccess: updateSuccess },
  ] = useUpdateAdminProductMutation();
    ...
    <>
        ...
        >
          ...
          <div className="mt-6">
            ...
            <input
              ...
              className={`input-md input mt-2 input-bordered w-full border-2 rounded-md focus:outline-none focus:ring focus:border-blue-300 ${
          ...
          <div>
            <div>
              <input
                type="file"
                onChange={(e) => {
                  setImgprint(URL.createObjectURL(e.target.files[0]));
          ...
          <button type="submit" className="btn btn-primary my-3 w-full">
            Submit
          </button>
        </form>
      </dialog>
            ...
            <tbody>
              {data &&
                data.map((item) => (
                  <tr
                    key={item.id}
                    className="border-2 dark:bg-gray-600 dark:text-white"
                  >
                    ...
                    <td className="p-0 flex gap-2">{item.size}</td>
                    <td className="p-0 w-40">
                      <p>{item.desc}</p>
                    </td>
                    <td className="p-0">
                      <button
                        className="btn text-white me-3 bg-yellow-400 hover:bg-yellow-500"
                        onClick={() => (
                          window.my_edit_3.showModal(), setHandleedit(item)
                        )}
                      >
                        Edit
                      </button>
      ...
      <dialog id="my_edit_3" className="modal mix-blend-normal">
        {handleedit && handleedit && (
          <div className="modal-box">
            <button
              onClick={() => window.my_edit_3.close()}
              className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
            >
              ✕
            </button>
            ...
            <div className="mt-2">
              <div className="flex gap-3 items-center">
                {imgopacity ? (
                  <>
                    <img
                      src={temimg && temimg.newHero}
                      className="w-48 h-40 object-cover"
                      alt=""
                    />
                    <button
                      onClick={() => setImgopacity(false)}
                      className="btn bg-red-400  hover:bg-red-600 mt-16 btn-sm"
                    >
                      Cancel
                    </button>
                    <input
            ...
            {/* Submit */}
            <button
              onClick={() => updateProduct(handleedit)}
              type="submit"
